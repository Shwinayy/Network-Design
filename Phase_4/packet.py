...
# type(1) seq(4) ack(4) length(2) flags(1) checksum(2)
HEADER_STRUCT = struct.Struct("!BIIHBH")
...
class Packet:
    ...
    def decode(cls, raw: bytes) -> "Packet":
        ...
        pkt_type, seq, ack, length, flags, checksum = HEADER_STRUCT.unpack(raw[:HEADER_SIZE])
        payload = raw[HEADER_SIZE:]
        if len(payload) != length:
            raise ValueError("payload length mismatch")
        zeroed = HEADER_STRUCT.pack(pkt_type, seq, ack, length, flags, 0) + payload
        if internet_checksum(zeroed) != checksum:
            raise ValueError("checksum mismatch")
        return cls(pkt_type=pkt_type, seq=seq, ack=ack, flags=flags, payload=payload)
